...
class GetMask(nn.Module):
    def __init__(self, threshold = 0.1592) -> None:
        ...
        self.register_buffer('threshold2', torch.FloatTensor([0.1]), persistent=False)
    ...
    def forward(self, sigma):
        ...
        # if self.training:
        #     threshold = h.view(B, -1).median(dim=-1, keepdim=True)[0]
        #     threshold.unsqueeze_(-1).unsqueeze_(-1)
        ...
        pool_pos = h <= self.threshold
        # pool_pos =  (h <= self.threshold) & (h >= self.threshold2)
...
class ContextIterator(nn.Module):
    ...
    def fill_pooling_value(x, values_mask, pool_conv):
        values = pool_conv(x * values_mask)
        return x + values
...
class ELIC_ADA(ELIC):
    def __init__(self, N=192, M=320, stage=CodecStageEnum.TRAIN, skipIndex=SKIP_INDEX) -> None:
        ...
        index = 5
        self.partial_skip = False
        print("skip threshold: ", index)
        print("skip index: ", self.skipIndex)
        print("partial skip: ", self.partial_skip)
    ...
    def forward(self, x):
        if self.stage == CodecStageEnum.TRAIN or self.stage == CodecStageEnum.TRAIN_ONEPATH:
            ...
            y_slices = self.y_entorpy.quantize(
                y, "noise" if self.training else "dequantize"
            )
            
            y_round = self.ste_quant(y)
            y_slice_iter = iter(self.sliceModel(y_slices))
            ...
            mu_hat = mu_hat * (1 - mask[0]) + mu * mask[0]
            sigma_hat = sigma_hat * (1 - mask[0]) + sigma * mask[0]
            ...
            values_mask = torch.cat(values_mask, 1)
            ...
            if self.partial_skip:
                y_slices_skip = y_slices * (1 - values_mask) + mu * values_mask
                y_slices = (y_slices + y_slices_skip) / 2
    ...
    def compress(self, x):
            ...
            non_anchors.append(non_anchor_index)
            ...
            y_string_list = []
            for i, y_slice, mu, sigma in zip(range(self.n_groups), self.sliceModel(y_slices), mu_list, sigma_list):
                if not no_sp_flag:
                    y1, y2 = y_slice[:, anchors[i][0]], y_slice[:, non_anchors[i][0]]
                    mu1, mu2 = mu[:, anchors[i][0]], mu[:, non_anchors[i][0]]
                    sigma1, sigma2 = sigma[:, anchors[i][0]], sigma[:, non_anchors[i][0]]
                else:
    ...
    def decompress(self, strings, shape):
        ...
        mu_hat = mu[:, anchor_index[0]]
        sigma_hat = sigma[:, anchor_index[0]]
        ...
        mu_hat = mu[:, non_anchor_index[0]]
        sigma_hat = sigma[:, non_anchor_index[0]]
